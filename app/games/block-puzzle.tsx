...
export type BlockPuzzleScreenContentProps = {
  embedded?: boolean;
  onBack?: () => void;
};

/** 게임 → 블록 퍼즐 */
export function BlockPuzzleContent({ embedded = false, onBack }: BlockPuzzleScreenContentProps) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={[]}>
      {onBack ? <WideSubpaneHeader title={t('gameBlockPuzzleTitle')} onBack={onBack} /> : null}
      <View
        style={[
          styles.fill,
          {
        ...
        <View style={styles.inner}>
          <BlockPuzzleGame wide={wide} split={split} fill viewportHeight={height} />
        </View>
      </View>
    </SafeAreaView>
  );
...
export default function BlockPuzzleScreen() {
  const { t } = useLocale();
