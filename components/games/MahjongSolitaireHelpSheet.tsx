...
export function MahjongSolitaireHelpSheet({ visible, onClose }: Props) {
      ...
      <View style={styles.backdrop}>
        ...
        <View style={[styles.sheet, { paddingBottom: Math.max(insets.bottom, 16) + 12 }]}>
          ...
          <ScrollView
            ...
            <Text style={styles.kicker}>{t('gameMahjongKicker')}</Text>
            <Text style={styles.blurb}>{t('gameMahjongBlurb')}</Text>
          </ScrollView>
        </View>
      </View>
    </Modal>
...
function makeStyles(theme: AppTheme, sf: (n: number) => number) {
  return StyleSheet.create({
    ...
    scroll: { ...BOTTOM_SHEET_SCROLL_STYLE },
    scrollContent: { paddingBottom: 12, gap: 10 },
    kicker: { fontSize: sf(14), fontWeight: '700', color: theme.green },
    blurb: { fontSize: sf(14), lineHeight: sf(21), color: theme.textMuted },
  });
}
