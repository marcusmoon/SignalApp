...
/** 홈 마감 브리핑 카드 → 바텀시트 (MarketBriefingSheet · EtfInsightSheet와 동일 셸) */
export function TodayBriefingSheet({ visible, briefing, onClose }: Props) {
  ...
  const title = useMemo(() => {
    ...
    return (
      ...
      briefing.title?.trim() ||
      t('todayBriefingDetailKicker')
    );
  }, [briefing, t]);

  const subtitle = useMemo(() => {
    if (!briefing?.briefingDate) return '';
    return formatLocalYmdLabel(briefing.briefingDate, locale, {
      month: 'short',
      day: 'numeric',
      weekday: 'short',
    });
  }, [briefing?.briefingDate, locale]);
          ...
          <ScrollView
            ...
            {briefing ? (
              <>
                ...
                <TodayBriefingBlock briefing={briefing} theme={theme} scaleFont={scaleFont} />
              </>
            ) : null}
          </ScrollView>
        </View>
