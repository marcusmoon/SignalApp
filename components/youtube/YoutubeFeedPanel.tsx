...
import { SCREEN_LIST_CONTENT_PADDING_TOP } from '@/constants/segmentTabBar';
import {
  webFlexFill,
...
export type YoutubeFeedPanelProps = {
  /**
...
/**
 * 유튜브 최신 리스트 + 채널 필터. 탭(`/youtube`)과 뉴스 YouTube 세그먼트가 공유한다.
 */
export const YoutubeFeedPanel = forwardRef<YoutubeFeedPanelHandle, YoutubeFeedPanelProps>(
  function YoutubeFeedPanel(
    {
      embedded = false,
  ...
  useImperativeHandle(ref, () => ({ refresh: () => void onRefresh() }), [onRefresh]);
  useRegisterWebHeaderRefresh(
    registerWebHeaderRefresh ? () => void onRefresh() : () => {},
  );
