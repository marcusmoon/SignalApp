...
| 시황/브리핑 카드 | 헤드라인 + 본문 **최대 2줄**(ellipsis). 전문은 상세 화면. 본문 뷰잉 시 `+1.2%`/`-0.5%` 등 **부호 있는 변동률만** 설정 색(한/미)으로 강조 |
| 노출 개수 | 설정 → 표시에서 조절 (기본 2, 상한 예: 4). 기본값을 화면에 하드코딩하지 말 것 |
| 상세 화면 | 말줄임 없이 전체 텍스트. 섹션당 한 목적 |
| 구분 | 행 구분선·여백으로 리듬. 카드 중첩·악센트 세로선 남발 금지 |
...
| 홈 시황 줄 수 | `services/homeMarketBriefingDisplayPreference.ts` |
