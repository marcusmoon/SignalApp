...
| 스캔 계층 | 홈은 피드 리스트보다 한 단계 큰 전용 스케일. 헤드라인 > 일정 > 뉴스 제목 > 메타. 상수로만 (`homeScan` / 동등) |
| 시황/브리핑 카드 | 헤드라인은 홈에서 줄 수 제한 없음(상세와 동일하게 핵심이 잘리게 두지 않음). 리스트형 미리보기만 최대 2줄. 본문 뷰잉 시 `+1.2%`/`-0.5%`·`2.78% 상승`·근처 방향 힌트가 있는 `2.78%` 등 **변동률**을 설정 색(한/미)으로 강조 (점유·비중 등 비변동 `%` 제외) |
