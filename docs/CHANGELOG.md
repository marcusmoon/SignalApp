...
- **오늘의 시그널**: 앱 첫 화면은 서버 `/v1/insights`가 내려주는 인사이트가 있으면 최신 뉴스보다 위에 `오늘의 시그널` 카드로 보여준다. 카드는 관련 기사/영상 출처로 자연스럽게 이동할 수 있어야 한다.
...
- **수집 메뉴 명명**: 좌측 수집 영역은 **콘텐츠 수집 관리**로 묶고, 하위 메뉴는 `수집 Job` / `실행 모니터링` / `실패·오류 로그` 흐름으로 둔다.
