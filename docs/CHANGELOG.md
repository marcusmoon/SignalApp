...
- **관심 브리핑 / 종목 상세 역할**: 관심 브리핑은 기기 관심종목 전체를 요약하는 개인 대시보드이며, 로컬 점수판은 `관심종목 점수`로 표시해 서버 인사이트인 `오늘의 시그널`과 구분한다. 종목 상세는 한 종목의 가격·뉴스·실적·신호 요약을 드릴다운하는 화면이다. 두 화면의 신호 사유/등락 계산 표시는 `utils/signalDisplay` 공통 기준을 사용한다.
