...
- **외부 뉴스 소스 확장**: 기본 Job에는 비활성 상태의 `SEC EDGAR 공시 수집`과 `GlobeNewswire 실적 RSS`가 포함된다. SEC EDGAR는 관심종목의 8-K/10-Q/10-K/S-1/6-K/20-F를 뉴스 촉매로 저장하며, 운영 환경에서는 `SEC_USER_AGENT`로 식별 가능한 User-Agent를 설정한다. GlobeNewswire는 범용 RSS 수집 handler(`newswire_rss`)를 통해 피드 URL·키워드·출처명을 Job params로 바꿔 확장할 수 있다.
