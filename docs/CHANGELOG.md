...
- **뉴스 UI**: `components/signal/NewsCard` — 해시태그·「원문 보기」푸터, 상대 시각은 **경과 시간** 기준(방금 / N분·시간 전; 로컬 날짜 전환과 무관). 뉴스 탭은 상단 헤더와 글로벌/코인/한국 세그먼트를 고정하고, 새로고침 시 현재 세그먼트 뉴스와 오늘의 시그널을 함께 최신화한다. 새 업데이트 안내는 오늘의 시그널 영역 아래, 세그먼트 탭 위에 표시한다.
...
- **관심 브리핑 / 종목 상세 역할**: 관심 브리핑은 기기 관심종목 전체를 요약하는 개인 대시보드이며, 로컬 점수판은 `관심종목 점수`로 표시해 서버 인사이트인 `오늘의 시그널`과 구분한다. `오늘 볼 종목`은 뉴스·변동·실적 일정이 있는 관심종목을 우선 추려 전체 관심목록 반복 나열을 줄인다. 종목 상세는 한 종목의 가격·뉴스·실적·신호 요약을 드릴다운하는 화면이며, 해당 티커의 서버 인사이트가 있으면 `오늘의 시그널` 카드로 함께 보여준다. 두 화면의 신호 사유/등락 계산 표시는 `utils/signalDisplay` 공통 기준을 사용한다.
