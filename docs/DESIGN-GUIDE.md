...
- **설정** (`app/settings.tsx`): My info에서 진입. `from=account`이면 상단 pill 서브탭 숨김(iPhone). 웹·iPad는 `ipadNav.showSettings` + `WideSubpaneHeader`(뒤로→내 정보). 탭 순서는 `constants/settingsTabs.ts`.
