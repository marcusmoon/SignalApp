...
- 홈 시세 그리드: **지수 6**(S&P · 나스닥 · 다우 · 필라 · 코스피 · 니케이) → 관심 종목 → 시총 상위 코인. 레이어마다 짧은 킥커(`지수`·`관심`·`코인`) + 간격 구분(바깥 카드 중첩·hairline 없음). 폰 **2열** · 와이드(웹/iPad) **3열**. 지수·코인 탭 → Yahoo Finance, 주식 탭 → 종목 상세. 지수 데이터는 Yahoo Job(`market_quotes_indices`). 지수 로고는 Parqet용 추종 ETF 심볼(`SPY`·`QQQ`·`DIA`·`SOXX`·`069500`·`EWJ`) — caret 지수 심볼은 CDN에 없음
