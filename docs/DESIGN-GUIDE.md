...
- **홈 섹션 순서**
  ...
  - **홈 바로가기** (`HomeShortcutsStrip`): 보드 목록·개별 게시글·시세·뉴스·IT 뉴스·일정·ETF·공시·설정 (기본 보드·시세·뉴스·일정, 최대 6). My info → 표시 → **홈 바로가기**에서 설정(개수 카드와 분리). 빈 선택이면 섹션 숨김
...
- 홈 설정(표시 탭, 카드 분리): **홈 바로가기**(목록·개별 게시글, 최대 6) · **홈 개수**(관심 종목·섹터 흐름·뉴스 흐름). 히어로·일정 칩은 자동
