...
- 홈 섹션 제목 네이밍 (표시 타이틀 · API 키 유지):
  ...
  - **홈 뉴스 흐름 NEW**: 섹션 헤더 우측(`trailingBadge`). 표시 목록 최신 `generatedAt`이 **1시간 이내**일 때만 (`isHomeNewsFlowNew`). 행마다 NEW 없음. 칩은 `theme.green` 단색 + **흰 글자** (`NewBadge`)
- **홈 섹션 순서**
...
- 홈 시세 그리드: 워치리스트 + 시총 상위 코인(별도 섹션 없음). 폰 **2열** · 와이드(웹/iPad) **3열**
...
- **더보기** (`app/(tabs)/more.tsx`): **iPhone만** — 내 정보·공시·**ETF**(짧은 라벨)·게시판·**게임** 숏컷 + 참고 링크 + 광고. 설정 메뉴는 없음.
  - **게임**: 진입·새 게임 추가·합 트레일 규칙은 [GAME-CENTER.md](./GAME-CENTER.md).
- **웹·iPad 사이드바**: More 항목 없음. 순서 — 홈 · 뉴스 · 시장 · 시세 · 공시 · **ETF** · 게시판 · **게임** · **내 정보**. 설정은 내 정보 허브에서 진입.
