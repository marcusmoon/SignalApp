...
- **본문 entity 링크** (장중·ETF·오늘 정리·뉴스/공시 다이제스트 공통): ingest 선택 필드 `entities: { name, symbol }[]`. 앱은 추측 스캔 없이 name 정확 매칭만. 심볼은 Yahoo 형식(US 그대로, KR은 `.KS`/`.KQ`, 지수 `^`, 크립토 `BTC-USD`). KR → Naver, 그 외 → Yahoo
