...
- **더보기** (`app/(tabs)/more.tsx`): **iPhone만** — 내 정보·공시·**섹터**(짧은 라벨)·**스크리너**·게시판·**게임** 숏컷 + 참고 링크 + 광고. 설정 메뉴는 없음.
  - **스크리너**: market(`kr`/`global`) 공용 풀 + method(예: 후지모토식 「후지모토 모멘텀」) 큐레이션 (`/screener?market=&method=`). 홈·하단 탭 상시 섹션 없음. 행 탭 → 종목 상세. 계약은 [SCREENER-AUTOMATION.md](./SCREENER-AUTOMATION.md).
