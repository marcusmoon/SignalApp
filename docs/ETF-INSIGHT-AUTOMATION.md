...
| 경로 | 동작 |
|---|---|
| **더보기** (메인) | 허브 타일 **섹터** → 리스트 `/etf-insights` (홈·상세 제목은 섹터 흐름) |
| **홈** (보조) | 선택일 기준 최신건이 **7일 이내**일 때만 「섹터 흐름」. 히트맵 미니뷰 우선. 빈 섹션·상시 고정 금지 |
| 홈 카드 탭 | 상세 `/etf-insight` (알림·리스트 행과 동일) |
| 리스트 행 | 상세 `/etf-insight` |
| 푸시·알림함 | ingest 시 발행 → 상세 `/etf-insight` |
...
**themes UI**: 테마명·모멘텀·요약만 표시(로고·하단 티커 목록 없음). 요약 본문은 텍스트만 — 티커 인라인 링크 없음.
