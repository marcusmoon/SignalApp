...
- `keywords`: 홈 스캔용 키워드. `{ label|symbol, kind?, weight?, name? }` 권장 (`kind`: `theme`|`symbol`|`macro`|`event`, 문서당 최대 6). **종목은 `kind:"symbol"` + 티커(`symbol`/`label`) + `name`(회사명)**. 문자열 배열도 ingest 가능(6자리 코드→symbol). 일반어(`시장`/`뉴스` 등) 금지. `topics`만 있으면 홈은 topics로 폴백
