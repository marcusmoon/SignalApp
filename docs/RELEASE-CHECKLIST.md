...
- [ ] 홈 장중 브리핑 히어로 카드 → `MarketBriefingSheet`(해당 회차만). 섹션 헤더 `>` 없음(시장 탭으로 탐색)
- [ ] 홈 오늘 정리 히어로 카드 → `TodayBriefingSheet`. 섹션 헤더 `>` 없음. 본문 없으면 히어로 미노출(장중 폴백 또는 섹션 숨김)
- [ ] 홈 뉴스·일정·관심·섹터 섹션 헤더에 `>` 없음(목록은 각 메뉴)
