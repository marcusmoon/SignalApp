...
다이제스트가 없는 세그먼트(뉴스 관심·YouTube, 공시 종목 필터 등)는 `SCREEN_LIST_CONTENT_PADDING_TOP`(12)을 쓴다. 뉴스 YouTube 세그먼트는 `YoutubeFeedPanel`을 **embedded**(채널 필터 스트립 숨김)로 쓴다. 채널 필터는 유튜브 탭에서만 노출한다.
...
- **설정**: More 탭에는 없음. 진입은 (1) **퀵 설정 → More settings** — 전체 pill 탭(iPhone·iPad), (2) **My info → 환경 설정** — `from=account`로 단일 탭(pill 숨김). 웹·iPad는 사이드바 **IT 뉴스 아래 내 정보**로 My info에 진입하고, 퀵 링크는 사이드바 하단 도크(`SidebarReferenceLinksDock`)에 둔다. More 허브 항목은 사이드바에 없음(IT 뉴스·공시·유튜브·보드는 사이드바 직접 항목).
...
| 진입 | 뒤로 크롬 |
|---|---|
| More → IT 뉴스·보드·공시·유튜브·My info | **같은 root Stack** — native 헤더 + `PhoneHeaderBackButton`(chevron만) |
