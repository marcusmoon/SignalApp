...
대상 예: 홈 → 뉴스·오늘 정리·장중 브리핑·캘린더·섹터 흐름·**보드 게시글·심볼**·내 정보 하위(설정·알림·약관) 및 그 안의 상세. 공시 플로우는 더보기/공시 탭에서 진입. 직접 `/community/…`·`/symbol/…`는 루트 진입(헤더 없음). 사이드바 **시장·섹터·스크리너·게시판·내 정보** 직접 진입은 드릴인이 아니므로 `WideSubpaneHeader` 없음. 게시글 백은 게시판 목록(`drillFrom: 'board'`)으로.
