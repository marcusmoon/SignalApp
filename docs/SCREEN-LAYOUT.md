...
- **단건 상세** (`today-briefing` · `news-digest` · `disclosure-digest` · `etf-insight` · 알림→`market-briefing`): Stack 제목·`dateBar` 없음(뒤로만). 알림 장중은 회차 세그먼트도 숨김. wide는 `WideSubpaneHeader` chevron만.
