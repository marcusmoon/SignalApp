...
- **Wide URL 동기화**: 홈·내 정보·설정·뉴스 이슈·공시 플로우·유튜브 정렬 등은 `IpadSidebarNavContext`가 **실제 라우트**로 맞춘다 (`useGlobalSearchParams`). 화면 안 필터·날짜도 쿼리에 기본값까지 명시한다. 사이드바 서브탭은 owner 단위로 clear해 탭 전환 경합으로 메뉴가 사라지지 않게 한다. 선택 하이라이트는 `setSubTabs(owner, tabs, activeKey)`로 목록과 함께 맞추고, 세그먼트 변경 시 effect cleanup으로 clear하지 않는다(클릭 후 선택 표시 유실 방지).
...
- **홈·More 드릴 헤더** (숏컷·뉴스/섹터 흐름·브리핑 상세 공통): phone은 `signalDrillStackOptions`(`PhoneHeaderBackButton` + 제목), wide는 `WideSubpaneHeader`(동일 chevron + 제목, 스크롤 밖). 단건 상세(`BriefingDetailShell`) 제목 = 섹션명(`chromeTitle`), 본문 헤드라인 = 단건 제목, 헤드라인 아래 **시간 메타**(발행 시각·장중 회차). `dateBar` 없음.
...
대상 예: 홈 → 뉴스·오늘 정리·장중 브리핑·캘린더·섹터 흐름·**보드 게시글·심볼**·내 정보 하위(설정·알림·약관) 및 그 안의 상세. 공시 플로우는 더보기/공시 탭에서 진입. 직접 `/community/…`·`/symbol/…`는 루트 진입(헤더 없음). 사이드바 **시장·섹터·게시판·내 정보** 직접 진입은 드릴인이 아니므로 `WideSubpaneHeader` 없음. 게시글 백은 게시판 목록(`drillFrom: 'board'`)으로.
...
| 상수 | px | 용도 |
|---|---:|---|
| `SCREEN_TAB_SCROLL_BOTTOM_BASE` | 12 | 홈·마켓·뉴스·시세 공통 — 상단 리스트 pad(`SCREEN_LIST_CONTENT_PADDING_TOP`)와 동일 |
...
FAB는 리스트 **위에 오버레이**한다. `paddingBottom`에 FAB 전체 높이(`SIZE+GAP`)를 넣지 않는다. 하단은 `tabBarHeight + inset + base`까지. 탭별 base 예외 없음.
...
// FAB가 있는 홈·뉴스·시세 — cushion만 (공통 base 12)
paddingBottom:
