...
- **날짜 바 패턴**: 홈·시장·캘린더·뉴스/공시 플로우 — `SignalDateNavigator`를 고정 스트립(`topFixed` / `fixedTop`) 안에 배치. **C1**: 바깥 셸 1개만(보더·`bgElevated`), 안쪽 화살표/날짜/오늘 컨트롤은 보더·배경 없이 Ionicons + 텍스트. 뉴스 플로우 리스트는 시장 브리핑과 같이 **날짜 바 → 세그먼트** 순서로 `topFixed`에 둔다.
