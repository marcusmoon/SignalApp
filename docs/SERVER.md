...
- 투자 캘린더 수집
- 컨콜 수집
- 국내주식 야간 참고가 수집: `market_quotes_kr_after_hours`는 Hyperliquid 파생상품 심볼을 매핑해 `kr_after_hours` 시세를 저장한다. 매핑이 실패해도 `yahooSymbol`(`005930.KS` 등)로 Yahoo 정규장 시세를 저장해 앱 야간 탭에서 종목이 누락되지 않게 한다. API 응답에는 `afterHoursAvailable`, `regularSession`, `official`, `notice`가 포함된다.
- 오늘의 시그널 생성
- 번역/보정
