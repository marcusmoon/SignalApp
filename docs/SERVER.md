...
- 뉴스 수집·보정 (`market_news_*`, RSS, SEC, DART).
  - **글로벌**: `market_news_global` (Finnhub general) + `market_news_financial_juice` (Financial Juice RSS). Finnhub는 매체·티커, FJ는 실시간 와이어.
  - **실적 PR RSS 제외**: `market_news_globenewswire_earnings`(Globe/PR)는 **비활성**. 실적 **일정**은 `calendar_earnings`(Finnhub).
...
| 구분 | 소스 | 저장·API |
|---|---|---|
| 주식·ETF | Parqet CDN URL을 서버가 합성(`symbolLogoUrl`) · **Admin upsert** (ingest도 quotes/disclosures/briefings 시 보조 upsert) | `symbol_profiles` → quotes/briefings/disclosures/news/ETF → `symbolMeta` (미등록이면 `null`) |
| 코인 | Yahoo 시세 Job이 심볼별 CDN URL을 `imageUrl`에 저장 (CoinCap icons; Yahoo에 로고 필드 없음) | Job `market_coins_top` → `coin_markets.payload.imageUrl` · `GET /v1/coins` `imageUrl`. 목록 순서는 Admin `crypto_symbols` (`listPosition`) |
...
- 공개 enrich 공통: `server/src/symbols/enrichSymbolMeta.mjs` (`enrichItemsWithSymbolMeta` · `loadSymbolProfilesForItems`) — quotes/news/disclosures/briefings/ETF
- ingest 시 프로필 기록: `server/src/symbols/ingestSymbolProfiles.mjs` (`marketQuotes` · `disclosures` · `marketBriefings`)
- **Admin 관리**: `/admin` → 시스템 → **종목 프로필** (`GET/POST/PATCH/DELETE /admin/api/symbol-profiles`) — 이름·로고 URL 등록·수정·삭제
- **홈 지수 로고**: caret 심볼(`^GSPC` 등)은 Parqet에 없어 추종 ETF 프로필(SPY·QQQ·DIA·SOXX·069500·EWJ)을 `V14`로 시드하고, quotes enrich가 `homeIndexSymbolMeta`로 `symbolMeta.logoUrl`을 붙인다 (`homeIndexLogos.mjs`)
