...
앱 공개 조회 API는 대량 데이터가 쌓여도 응답 시간을 유지하기 위해 가능한 한 전체 `readDb()` 스냅샷을 만들지 않습니다. `/v1/news`, `/v1/news-sources`, `/v1/youtube`, `/v1/calendar`, `/v1/concalls`, `/v1/market-quotes`, `/v1/coins`, `/v1/market-lists`, `/v1/notifications`는 필요한 SQLite 테이블만 직접 조회합니다. `readDb()`는 여전히 관리 화면의 복합 요약이나 Job 실행처럼 전체 스냅샷이 필요한 흐름에서 사용합니다.
