...
- topics: 대표 주제 태그
- keywords: 홈 스캔용. 이슈당 최대 6개. `{ "label"|"symbol", "kind", "weight", "name"? }` 권장.
  - kind: "theme" | "symbol" | "macro" | "event"
  - 종목: `kind:"symbol"`, 티커는 `symbol`(또는 label), **회사명은 `name`** (예: `{ "kind":"symbol", "symbol":"005930", "name":"삼성전자" }`)
