...
function localizedConcallFallback(
  ...
  const dash = '—';
  if (kind === 'missing_key') return [row.callsAiAnthropicKeyMissing, dash];
...
async function postAnthropicMessage(system: string, user: string, maxTokens = 8192): Promise<string> {
  return postAnthropicMessages({ model: MODEL, system, user, maxTokens });
}

export function newsItemFromFinnhubFallback(n: FinnhubNewsRaw, locale: AppLocale): NewsItem {
...
async function translateNewsTitleWithClaude(article: FinnhubNewsRaw, locale: AppLocale): Promise<string | null> {
  try {
    ...
    const text = (await postAnthropicMessage(system, user, 256)).trim();
    if (!text) return null;
...
export async function summarizeConcallTranscript(
  ...
  let obj: ConcallJson;
  try {
    const text = await postAnthropicMessage(system, user, 4096);
