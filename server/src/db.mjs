...
import { upsertSymbolProfilesFromIngest } from './symbols/ingestSymbolProfiles.mjs';
import {
...
const collectionSpecs = [
  ...
  },
  {
    ...
    columns: (row, index) => ({
      ...
      generated_date: dateOrNull(row.generatedDate || row.asOf || row.publishedAt),
      as_of: isoOrNull(row.asOf || row.generatedAt),
      published_at: isoOrNull(row.publishedAt || row.generatedAt),
      updated_at: isoOrNull(row.updatedAt) || nowIso(),
    }),
  },
    ...
    await withKyselyTransaction(async (client) => {
      try {
        ...
        if (collectionKey === 'marketQuotes' || collectionKey === 'disclosures' || collectionKey === 'marketBriefings') {
          await upsertSymbolProfilesFromIngest(client, collectionKey, safeRows);
        }
      } catch (error) {
        throw error;
      }
