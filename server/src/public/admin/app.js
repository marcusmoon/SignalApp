            ...
            if (afterRows.length > 0) {
              const instruments = afterRows
                .map((row) => {
                  ...
                  const displaySymbol = String(row.querySelector(`[data-job-after-display="${key}"]`)?.value || '').trim();
                  ...
                  if (!symbol && !name && !displaySymbol && !yahooSymbol && candidates.length === 0 && regularCloseKrw == null) return null;
                  return {
                    symbol,
                    name,
                    displaySymbol,
